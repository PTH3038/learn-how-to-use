...
struct Node* anti_copyNode(struct Node* nSource)
{
	struct Node* p = (struct Node*)malloc(sizeof(struct Node));
	p->heSo = -nSource->heSo;
	p->soMu = nSource->soMu;
...
struct Node* addPolynomial(struct Node* first, struct Node* second)
{
	...
	struct Node* last_copy = NULL;
	struct Node* pointer = first;
	while (pointer) {
		addElementLast(&first_copy, &last_copy, copyNode(pointer));
		pointer = pointer->next;
	}

	addPolynomialToCurrentPlynomial(&first_copy, second);
	return first_copy;
}
	...
	while (first != NULL && second != NULL) {
		...
		if (first->soMu > second->soMu) {
			...
			if (first->heSo + second->heSo != 0) {
				...
				first = first->next;
				if (preFirst) preFirst->next = first;
				else *target = first;
			}
			second = second->next;
		}
...
{
	struct Node* second_copy = NULL;
	struct Node* last_copy = NULL;
	struct Node* pointer = second;
	while (pointer) {
		addElementLast(&second_copy, &last_copy, anti_copyNode(pointer));
		pointer = pointer->next;
	}

	addPolynomialToCurrentPlynomial(&second_copy, first);
	return second_copy;
}
...
struct Node* multiplyPolynomialWithNode(struct Node* head, struct Node* multiplier)
{
	if (head == NULL) return NULL;
	else if (multiplier == NULL) return head;
	else {
		...
		while (pointer) {
			addElementLast(&ans, &last, createNewNode(pointer->heSo*multiplier->heSo, pointer->soMu + multiplier->soMu));
			pointer = pointer->next;
		}
	return ans;
	}
...
struct Node* multiplyTwoPolynomial(struct Node* first, struct Node* second)
{
	...
	struct Node* ans = multiplyPolynomialWithNode(second, pointer);
	pointer = pointer->next;
	while (pointer) {
		ans = addPolynomial(ans, multiplyPolynomialWithNode(second, pointer));
		pointer = pointer->next;
	}
	return ans;
}
...
int main()
{
	...
	int polyIndex;  // 1 hoac 2
	struct Node *hPoly1 = NULL, *hPoly2 = NULL;

	struct Node *hPoly3, *aNode;
	double x;
